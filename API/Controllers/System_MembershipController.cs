...
namespace TA.API.Controllers
{
    public class System_MembershipController : BaseController
    {
        ...
        public int Add(System_Membership system_Membership)
        {
            ...
            if (system_Membership.ID == 0)
            {
                result = _system_MembershipRepository.Add(system_Membership);
            }
            else
            {
                result = _system_MembershipRepository.Update(system_Membership);
            }
            return result;
        }
        [HttpPost]
        ...
        [HttpPut]
        public int Update001(System_Membership system_Membership)
        {
            var result = AppGlobal.InitializationNumber;
            System_Membership model = _system_MembershipRepository.GetByID(system_Membership.ID);
            if (model != null)
            {
                model.Email = system_Membership.Email;
                model.Password = system_Membership.Password;
                model.DisplayName = system_Membership.DisplayName;
                result = _system_MembershipRepository.Update(system_Membership);
            }
            return result;
        }
        ...
        [HttpGet]
        public System_Membership GetByID(int ID)
        {
            System_Membership result = new System_Membership();
            if (ID > 0)
            {
                result = _system_MembershipRepository.GetByID(ID);
                if (result == null)
                {
        ...
        {
            var result = _system_MembershipRepository.GetBySearchStringToList(searchString);
            return result;
        }
        [HttpGet]
        public System_Membership AuthenticationByEmailAndPasswordAndURL(string email, string password, string urlDestination)
        {
            ...
            {
                urlDestination = urlDestination.Split('=')[urlDestination.Split('=').Length - 1];
                urlDestination = urlDestination.Replace(@"%2F", @"/");
                result = _system_MembershipRepository.AuthenticationByEmailAndPasswordAndURL(email, password, urlDestination);
            }
            return result;
        }
    }
