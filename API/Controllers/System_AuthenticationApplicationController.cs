...
namespace TA.API.Controllers
{
    public class System_AuthenticationApplicationController : BaseController
    {
        ...
        public int Add(System_AuthenticationApplication system_AuthenticationApplication)
        {
            ...
            if (system_AuthenticationApplication.ID == 0)
            {
                result = _system_AuthenticationApplicationRepository.Add(system_AuthenticationApplication);
            }
            else
            {
                result = _system_AuthenticationApplicationRepository.Update(system_AuthenticationApplication);
            }
            return result;
        }
        [HttpPost]
        ...
        [HttpGet]
        public System_AuthenticationApplication GetByID(int ID)
        {
            System_AuthenticationApplication result = new System_AuthenticationApplication();
            if (ID > 0)
            {
                result = _system_AuthenticationApplicationRepository.GetByID(ID);
                if (result == null)
                {
                    result = new System_AuthenticationApplication();
                }
            }
            return result;
        }
        ...
        {
            var result = _system_AuthenticationApplicationRepository.GetDataTransferBySearchStringToList(searchString);
            return result;
        }
    }
}
