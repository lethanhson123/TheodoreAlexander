...
function onViewUS() {
    ...
    if(document.getElementById("TAStudioFrenzyUSMobile")){
    document.getElementById("TAStudioFrenzyUSMobile").hidden = true;    
    }
     if(document.getElementById("TAStudioHolliUSMobile")){
...
function onCollectionMenuItems() {
    var url = "https://api.theodorealexander.com/api/Collection/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onTypeMenuItems() {
    var url = "https://api.theodorealexander.com/api/Type/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onLifeStyleMenuItems() {
    var url = "https://api.theodorealexander.com/api/LifeStyle/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onStyleMenuItems() {
    var url = "https://api.theodorealexander.com/api/Style/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onShapeMenuItems() {
    var url = "https://api.theodorealexander.com/api/Shape/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onRoomMenuItems() {
    var url = "https://api.theodorealexander.com/api/RoomAndUsage/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onBrandMenuItems() {
    var url = "https://api.theodorealexander.com/api/Brand/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onDesignerMenuItems() {
    var url = "https://api.theodorealexander.com/api/Designer/GetByIsActiveToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onClickMobileFilterIcon(isView) {
    ...
    if (isView == false) {
        ...
        document.getElementById("ProductListMenu").className = 'productFilter_sidebar ta_no_scrollbar d-none d-xl-none flex-column productFilter_sidebar_mobile-open';
    }
    else {
        document.getElementById("ProductListMenu").className = '';
        document.getElementById("ProductListMenu").className = 'productFilter_sidebar ta_no_scrollbar d-none d-xl-none flex-column';
    }
}
...
function onEXTENDINGChangeUS() {
    ...
    onSwitchToProductListing("Extending");
}
...
function onClearAll() {
    ...
    onSwitchToProductListing("ClearAll");
}
...
function onCollectionSelect(ID, URLCode, action) {
    ...
    onSwitchToProductListing("CollectionIds");
}
function onLifeStyleSelect(ID, URLCode, action) {
    ...
    onSwitchToProductListing("LifeStyleIds");
}
function onStyleSelect(ID, URLCode, action) {
    ...
    onSwitchToProductListing("StyleIds");
}
function onShapeSelect(ID, URLCode, action) {
    ...
    onSwitchToProductListing("ShapeIds");
}
function onTypeSelect(ID, URLCode, action) {
    ...
    onSwitchToProductListing("TypeIds");
}
...
function onSwitchToProductListing(countBy) {
    var url = "https://theodorealexander.com/product-listing;OrderBy=IsNew;DefaultItemOnly=true;countBy=" + countBy;
    if (countBy == "ClearAll") {
        url = "https://theodorealexander.com/product-listing;OrderBy=IsNew;requestCount=1;DefaultItemOnly=true";
        window.location.href = url;
    }
    if (countBy == "Extending") {
        url = url + ";Extending=true";
    }

    var requestCount = 0;

    var requestCountType = 0;
    var typeIds = "";
    var type_IDList = window.localStorage.getItem('type_IDList');
    var type_IDListCount = type_IDList.split(";");
    var typeList = JSON.parse(window.localStorage.getItem("TypeList"));
    for (let i = 0; i < typeList.length; i++) {
        for (let j = 0; j < type_IDListCount.length; j++) {
            if (typeList[i].URLCode == type_IDListCount[j]) {
                requestCountType = requestCountType + 1;
                typeIds = typeIds + typeList[i].ID.toUpperCase() + ",";
            }
        }
    }
    if (requestCountType > 0) {
        typeIds = ";TypeIds=" + typeIds;
        typeIds = typeIds.substr(0, typeIds.length - 1);
        url = url + typeIds;
        requestCount = requestCount + requestCountType;
    }

    var requestCountCollection = 0;
    var collectionIds = "";
    var collection_IDList = window.localStorage.getItem('collection_IDList');
    var collection_IDListCount = collection_IDList.split(";");
    var collectionList = JSON.parse(window.localStorage.getItem("CollectionList"));
    for (let i = 0; i < collectionList.length; i++) {
        for (let j = 0; j < collection_IDListCount.length; j++) {
            if (collectionList[i].URLCode == collection_IDListCount[j]) {
                requestCountCollection = requestCountCollection + 1;
                collectionIds = collectionIds + collectionList[i].ID.toUpperCase() + ",";
            }
        }
    }
    if (requestCountCollection > 0) {
        collectionIds = ";CollectionIds=" + collectionIds;
        collectionIds = collectionIds.substr(0, collectionIds.length - 1);
        url = url + collectionIds;
        requestCount = requestCount + requestCountCollection;
    }

    var requestCountLifeStyle = 0;
    var lifeStyleIds = "";
    var lifeStyle_IDList = window.localStorage.getItem('lifeStyle_IDList');
    var lifeStyle_IDListCount = lifeStyle_IDList.split(";");
    var lifeStyleList = JSON.parse(window.localStorage.getItem("LifeStyleList"));
    for (let i = 0; i < lifeStyleList.length; i++) {
        for (let j = 0; j < lifeStyle_IDListCount.length; j++) {
            if (lifeStyleList[i].URLCode == lifeStyle_IDListCount[j]) {
                requestCountLifeStyle = requestCountLifeStyle + 1;
                lifeStyleIds = lifeStyleIds + lifeStyleList[i].ID.toUpperCase() + ",";
            }
        }
    }
    if (requestCountLifeStyle > 0) {
        lifeStyleIds = ";LifeStyleIds=" + lifeStyleIds;
        lifeStyleIds = lifeStyleIds.substr(0, lifeStyleIds.length - 1);
        url = url + lifeStyleIds;
        requestCount = requestCount + requestCountLifeStyle;
    }

    var requestCountShape = 0;
    var shapeIds = "";
    var shape_IDList = window.localStorage.getItem('shape_IDList');
    var shape_IDListCount = shape_IDList.split(";");
    var shapeList = JSON.parse(window.localStorage.getItem("ShapeList"));
    for (let i = 0; i < shapeList.length; i++) {
        for (let j = 0; j < shape_IDListCount.length; j++) {
            if (shapeList[i].URLCode == shape_IDListCount[j]) {
                requestCountShape = requestCountShape + 1;
                shapeIds = shapeIds + shapeList[i].ID.toUpperCase() + ",";
            }
        }
    }
    if (requestCountShape > 0) {
        shapeIds = ";ShapeIds=" + shapeIds;
        shapeIds = shapeIds.substr(0, shapeIds.length - 1);
        url = url + shapeIds;
        requestCount = requestCount + requestCountShape;
    }

    var requestCountStyle = 0;
    var styleIds = "";
    var style_IDList = window.localStorage.getItem('style_IDList');
    var style_IDListCount = style_IDList.split(";");
    var styleList = JSON.parse(window.localStorage.getItem("StyleList"));
    for (let i = 0; i < styleList.length; i++) {
        for (let j = 0; j < style_IDListCount.length; j++) {
            if (styleList[i].URLCode == style_IDListCount[j]) {
                requestCountStyle = requestCountStyle + 1;
                styleIds = styleIds + styleList[i].ID.toUpperCase() + ",";
            }
        }
    }
    if (requestCountStyle > 0) {
        styleIds = ";StyleIds=" + styleIds;
        styleIds = styleIds.substr(0, styleIds.length - 1);
        url = url + styleIds;
        requestCount = requestCount + requestCountStyle;
    }

    var isStocked = window.localStorage.getItem('isStocked');
    if (isStocked == "true") {
        url = url + ";IsStocked=true";
    }
...
function getByQueryString001() {
    ...
    var url = "https://api.theodorealexander.com/api/Item/AsyncGetDataTransferByUser_IDAndIsActiveTAUSAndTailorFitProgramToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
        ...
        success: function (data) {
            ...
            window.localStorage.setItem("ProductListData", JSON.stringify(listProducts));
            onSortBy(1);
            onGetMenuLeft();
            document.getElementById("ProductListLoader").hidden = true;
        },
...
function onSortBy(sortBy) {
    ...
    if (listProducts==null) {  
        onInitializationProductListData(); 
    }
    else{
    ...
    if (listProducts) {         
        document.getElementById("ProductListUSResults").innerHTML = listProducts.length;
        document.getElementById("ProductListInternationalResults").innerHTML = listProducts.length;
        if (listProducts.length == 0) {
            ...
            switch (sortBy) {
                case "1":
                    listProducts = listProducts.sort(GetSortOrderDESC("IsNew"));
                    break;
                case "2":
                    listProducts = listProducts.sort(GetSortOrderDESC("IsBestSeller"));
                    break;
                case "3":
                    listProducts = listProducts.sort(GetSortOrderASC("ProductName"));
                    break;
                case "4":
                    listProducts = listProducts.sort(GetSortOrderDESC("ProductName"));
                    break;
            }
            initializationProductListHTML(listProducts);
        }
    }
    else {
        //document.getElementById("ProductListResultEmpty").hidden = false;
...
function initializationProductListHTML(list) {
    ...
    for (var i = 0; i < count; i++) {
        productListHTML = productListHTML + "<div class='product-tailor'>";
        productListHTML = productListHTML + "<div class='tags'>";
        if (list[i].IsCFPItem) {
            productListHTML = productListHTML + "<img src='https://theodorealexander.com/assets/img/CP-icon.png' class='pic-icon-cp'>";
        }
        if (list[i].IsAXHCFPItem) {
            productListHTML = productListHTML + "<img src='https://theodorealexander.com/assets/img/icon-AXH.png' class='pic-icon-axh'>";
        }
        if (list[i].IsNew) {
        ...
        productListHTML = productListHTML + "<a href='https://theodorealexander.com/product-detail/" + list[i].URLCode + ".html' title='" + list[i].ProductName + "'>";
        productListHTML = productListHTML + "<img src='" + list[i].Image + "' title='" + list[i].ProductName + "' alt='" + list[i].ProductName + "' />";
...
function onInitializationProductListData() {
    ...
    window.localStorage.setItem("ProductListData", null);
    var url = "https://api.theodorealexander.com/api/Item/AsyncGetDataTransferByUser_IDAndIsActiveTAUSAndTailorFitProgramToList";
    $.ajax({
        type: "GET",
        url: url,
        ...
        success: function (data) {
            ...
            if (user_ID != '00000000-0000-0000-0000-000000000000') {
                for (var i = 0; i < listProducts.length; i++) {
                    if (listProducts[i].IsInWishList) {
                        var itemIDWishlist = listProducts[i].ID.toUpperCase() + "Wishlist";
                        if(document.getElementById(itemIDWishlist)){
...
function onAddToWishlist(index) {
    ...
    if (list) {
        ...
        if ((userID) && (wishListID) && (itemID)) {
            var url = "https://api.theodorealexander.com/api/WishList/AddItemToWishListByUserIDAndWishListIDAndItemID";
            $.ajax({
                type: "GET",
                url: url,
                data: {
...
function onSaveToWishlist() {
    ...
    if (wishlistName) {
        ...
        var url = "https://theodorealexander.com/checklogin/" + URLCheck + "_" + itemIndex;
        if ((userID == null) || (userID.length == 0) || (userID.toUpperCase() == "UNDEFINED") || (userID.toUpperCase() == "00000000-0000-0000-0000-000000000000")) {
            window.location.href = url;
        }
        if (userID) {
            var url = "https://api.theodorealexander.com/api/WishList/AddWishListByUserIDAndWishlistName";
            $.ajax({
                type: "GET",
                url: url,
                data: {
...
function onGetWishlistByUserID() {
    ...
    if (userID) {
        var url = "https://api.theodorealexander.com/api/WishList/GetWishListByUserID";
        $.ajax({
            type: "GET",
            url: url,
            data: {
...
function onGetMenuLeft() {
    ...
    var url = "https://api.theodorealexander.com/api/Item/AsyncGetItemMenuLeftDataTransferByUser_IDAndIsActiveTAUSAndFiltersToList";
    $.ajax({
        type: "GET",
        url: url,
        data: {
...
function onCheckMenuLeft() {
    ...
    if (collectionCount == 0) {
        ...
        if(document.getElementById(MenuLeftID)){
            document.getElementById(MenuLeftID).hidden = false;
        }        
        document.getElementById(regionFilter).hidden = false;
        document.getElementById(region001).hidden = true;
